...
|---|---|---|---|
| MinerU cloud OCR (default / recommended) | User passes `--ocr` or the file is an image / scanned PDF and they said "recognize the text". | Free for normal use, MINERU_TOKEN unlocks bigger files. | `prefer_mineru_sdk=True` (default), network available, mineru-open-sdk installed.  Passed via SDK param `is_ocr` (Flash) or `ocr` (Precision). |
